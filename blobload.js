 ...
 * @type{{
 *      buffer: {},
 *      httpAccessStatusCode: number,
 *      httpAccessContent: string,
 *      port: number,
 *      sliceSize: number,
 *      dataBufferLimit: number,
 *      tempFolderPath: string,
 *      storageFolderPath: string,
 *      httpServer: null,
 *      currentTotalBufferSize: number,
 *      maxConnectionsAllowed: number,
 *      currentNbOfConnections: number,
 *      bufferBackupTimeInSeconds: number,
 *      getHttpAccessStatusCode: Function,
 *      getHttpAccessContent: Function,
 *      processNewUpload: Function,
 *      processSlice: Function,
 *      setup: Function,
 *      stop: Function,
 *      start: Function,
 *      getFileLocation: Function,
 *      setHttpAccessData: Function,
 *      setPort: Function
 *  }}
 */
var BlobloadServer = {
    ...
    /**
     * The HTTP protocol to use for the server, can be "http" or "https"
     * @type {string}
     */
    httpProtocol: "http",

    /**
     * The options for setting up the HTTP server
     * @type {object}
     */
    httpOptions: {},
     ...
     * @type {number}
     *
     * TODO: evaluate the need for cache cleaning when buffer size is almost at its limits. Disabling this functionality
     * for now.
     */
    ...
    /**
     * The amount of time to keep uploaded data in the buffer before cleanup after the client disconnects before the
     * upload is finished (in seconds).
     */
    bufferBackupTimeInSeconds: 3600,

    /**
     * Create and configure a blobload server.
     *
     * @param httpProtocol
     *          The protocol to be used for the HTTP server. Value is either "http" or "https".
     * @param httpOptions
     *          The options for setting up the HTTP server. The "cert" and "key" entries are required for an HTTPS
     *          server.
     * @param port
     *          The port on which this blobload instance will run and listen.
     * @param sliceSize
     *          The preferred size (in bytes) of slices to use.
     ...
     *          The amount of time to keep unfinished upload data in the buffer before deleting them after a client
     *          disconnects.
     * @param tempFolderPath
     *          The path of the directory to use to store currently uploading files (absolute or relative to the
     *          directory this module is in).
     * @param storageFolderPath
     *          The path of the directory to use to store finished uploads (absolute or relative to the directory this
     *          module is in).
     * @param authorizer
     *          The authorizer function to use for authentication.
     *          The token received from the client is passed on to this authorizer function as the first parameter.
     *          The second parameter passed on to the authorizer function is the callback function to execute after
     *          validating the token. The callback function has the following signature: function(string, boolean). The
     *          first string parameter represents the error message if validation failed (null otherwise). The second
     *          boolean parameter represents the result of the validation. So if validation fails, due to an invalid
     *          token for instance, a possible usage of the callback function could be:
     *          callback("Invalid token", false).
     *
     * @returns {BlobloadServer} This blobload server (for method chaining purposes).
     */
    setup: function(httpProtocol, httpOptions, port, sliceSize, dataBufferLimit, maxConnections, bufferBackupTimeInSeconds, tempFolderPath, storageFolderPath, authorizer)
    {
        // Set the properties
        ...
        if(dataBufferLimit > this.sliceSize)
            this.dataBufferLimit = dataBufferLimit;

        if(maxConnections > 0)
            this.maxConnectionsAllowed = maxConnections;

        if(bufferBackupTimeInSeconds > 0)
            this.bufferBackupTimeInSeconds = bufferBackupTimeInSeconds;

        if(tempFolderPath)
            this.tempFolderPath = (tempFolderPath.charAt(tempFolderPath.length - 1) == '/') ? tempFolderPath : (tempFolderPath + '/');

        if(storageFolderPath)
            this.storageFolderPath = (storageFolderPath.charAt(storageFolderPath.length - 1) == '/') ? storageFolderPath : (storageFolderPath + '/');

        // Create the HTTP server module
        httpModule = require(httpProtocol);
        ...
        this.httpServer = httpModule.createServer(httpOptions, function(req, res)
        {
        ...
        // Activate the socket.io module and initialize a socket to listen for events from the HTTPS server
        var ioServer = io.listen(this.httpServer);

        // configure the I/O server
        ...
        for(var file in this.buffer)
        {
            ...
            if(Date.now() > expiryDate)
            {
                ...
                fs.unlink(fileLocation, function(err)
                {
                    if(err)
                    {
                        console.log('Error occurred while deleting "' + fileLocation + '" after buffer cleanup for that upload');
                    }
                });
            }
        }
